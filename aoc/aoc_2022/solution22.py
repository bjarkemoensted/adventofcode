...
def parse(s: str) -> tuple[NDArray[np.str_], list[str|int]]:
    map_str, instructions_raw = s.split("\n\n")

    map_ = map_str.splitlines()
    rows = len(map_)
...
def _add_tuples(a: coordtype, b: coordtype) -> coordtype:
    assert len(a) == len(b)
    r1, r2 = tuple(c1 + c2 for c1, c2 in zip(a, b))
    return r1, r2


def _multiply_tuple(n: int, tup: coordtype) -> coordtype:
    i, j = tuple(n*x for x in tup)
    return i, j


def _neighbor_offsets(dim: int|None=None, include_diagnonal=True, initial_pos=None) -> tuple[coordtype, ...]:
...
class State:
    ...
    def __init__(self, pos: tuple, dir_: tuple | None, custom_symbol: str|None = None):
...
class Map:
    ...
    def __init__(self, layout: NDArray[np.str_]):
    ...
    def attempt_single_step(self, state: State) -> State:
    ...
    def step(self, state: State, n_steps: int) -> State:
        """Takes the input number of steps"""

        res: State|None = None
        for _ in range(n_steps):
            res = self.attempt_single_step(state)
            if res == state:
                break
            state = res
    ...
    def move(self, state: State, instruction: str | int) -> State:
        ...
        if isinstance(instruction, str) and instruction in ("L", "R"):
...
def solve(data: str) -> tuple[int|str, ...]:
    layout, instructions = parse(data)
