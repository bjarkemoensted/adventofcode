...
@cache
def rotate(dir_: coordtype) -> tuple[coordtype, coordtype]:
...
def get_neighbors(M: NDArray[np.str_], i: int, j: int, *directions):
...
def build_graph_from_sketch(M: NDArray[np.str_]):
    ...
    nrows, ncols = M.shape
    ij2neighbors: dict[coordtype, list[coordtype]] = dict()
    _coords_sentinel = (-1, -1)
    S_coords = _coords_sentinel
    ...
    if S_coords == _coords_sentinel:
        raise RuntimeError("Couldn't locate placeholder symbol")
    
    # Construct graphs for all possible values of S
    longest_loop = float("-inf")
    G_best = None
    for _, directions in _char2directions.items():
        i_s, j_s = S_coords
...
def find_distance(G, coords) -> int:
...
def get_blop(M: NDArray[np.str_], i: int, j: int, forbidden_sites: set[coordtype]):
...
def _map_blop_to_nodes(node2blop: dict[coordtype, int]) -> dict[int, set[coordtype]]:
    ...
    blop2nodes: dict[int, set[coordtype]] = dict()
