...
def parse(s: str) -> str:
    res = "".join(mapping[char] for char in s)
    return res
...
class OperatorParser(Parser):
    ...
    def scan_next_subpacket(self):
        ...
        constructor = LiteralValueParser if next_packet_is_literal_value(substring) else OperatorParser
        subparser = constructor(s=substring, verbose=self.verbose, level=self.level + 1, remove_trailing_zeroes=False)
