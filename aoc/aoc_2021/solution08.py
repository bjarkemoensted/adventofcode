...
def parse(s: str) -> tuple[list[list[str]], ...]:
    digits_list = []
    displays_list = []
    ...
    #
    return digits_list, displays_list
...
def brute(locked: dict[str, str], free: dict[str, set[str]]) -> Iterator[dict[str, str]]:
    ...
    for char in list(free.keys()):
        for candidate in sorted(free[char]):
            ...
            yield from brute(locked, new_free)
        #
    #


def decode(digs: list[str], mapping: dict[str, str]) -> list[str]:
...
def crack(digs: list[str], count2possible_letters: dict[int, str]) -> dict[str, str]:
    """Determine most of the mapping from segments on broken display to intended segment
    from number of digits in which the segments partake"""
...
def solve(data: str) -> tuple[int|str, ...]:
    digits_list, displays_list = parse(data)

    # Map each letter to the letters they must co-occur with
    chars = get_unique_chars(digit2segments.values())
    cooccurrences: dict[str, set[str]] = {char: set([]) for char in sorted(chars)}
    ...
    # Count number of occurrences of digits 1,4,7, and 8 (which have unique lengths)
    target_digits = [1, 4, 7, 8]
    target_lengths = {len(digit2segments[k]) for k in target_digits}

    star1 = sum(len(s) in target_lengths for s in sum(displays_list, []))
    ...
    for digits, displays in zip(digits_list, displays_list):
        ...
        mapping = crack(digits, count2possible_letters)
